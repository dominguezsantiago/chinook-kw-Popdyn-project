...
The indexes were calculated according to the lipid ranking and the lenght-lipid relationship parameters found in _O'neill et al., 2014_:
...
```{r x1, warning=FALSE}
lipids <- left_join(full_gam_predictions,lipid_ranking_stock, by = c("region"= "RMIS.Region"))
```

```{r x2, echo=FALSE, warning=FALSE}
lipids <- lipids%>%
...
```{r filling gaps 2, warning=FALSE, fig.align='center', fig.width=20, fig.height=30, echo=FALSE}
...
```{r filling gaps plot1 2, echo = FALSE, warning=FALSE,fig.align='center', fig.width=20, fig.height=30}
ggplot(lipids.full, aes(x=as.factor(age), y = cohort, fill = !is.na(lipid_content)))+
  geom_tile(color = "black")+
  facet_wrap(~fram.long.names, nrow = 10)+
  labs(x= "Age", y = "Run year")+
...
```{r filling gaps plot2 2, echo = FALSE, warning=FALSE,fig.align='center', fig.width=20, fig.height=30}
